...
#pragma warning disable CS0618 // Type or member is obsolete
        ...
        [Obsolete($"Check that T actually has a [GenerateShape] attribute or otherwise implements or is constrained to IShapeable<T>. If T is declared in an assembly that does not target .NET, use {nameof(TypeShapeResolver)}.{nameof(TypeShapeResolver.ResolveDynamic)}<T>() instead.", error: true)]
#endif
...
#if NET
        [EditorBrowsable(EditorBrowsableState.Never)]
        [Obsolete($"Check that TProvider actually has a [GenerateShape] attribute or otherwise implements or is constrained to IShapeable<T>. If TProvider is declared in an assembly that does not target .NET, use {nameof(TypeShapeResolver)}.{nameof(TypeShapeResolver.ResolveDynamic)}<T, TProvider>() instead.", error: true)]
#endif
