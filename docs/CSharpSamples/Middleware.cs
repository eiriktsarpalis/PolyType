...
internal static class ShapeProcessorExtensions
{
    extension(ShapeProcessor)
    {
        ...
        [Obsolete("Use the ShapeProcessor.ConsumeShape<T>() method instead. If using the extension method syntax, check that T actually has a [GenerateShape] attribute or otherwise implements or is constrained to IShapeable<T> to avoid a runtime failure.", error: true)]
#endif
...
#if NET
        [EditorBrowsable(EditorBrowsableState.Never)]
        [Obsolete("Use the ShapeProcessor.ConsumeShape<T, TProvider>() method instead. If using the extension method syntax, check that T actually has a [GenerateShape] attribute or otherwise implements or is constrained to IShapeable<T> to avoid a runtime failure.", error: true)]
#endif
